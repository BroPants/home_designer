...
2. **后端**:
   ...
   - `clear_conversation` 命令支持清空对话
   - 对话历史自动保存到数据库
   - 支持上下文记忆（最近 10 条消息）
   - 流式响应框架已搭建（`chat_stream`）

### Phase 4: 效果图生成 (Week 4-5) 🚧 进行中
...
**主要完成内容**:
1. **前端**:
   - RenderingViewer 支持本地图片路径显示
   - 添加生成效果图按钮（空状态和已有效果图状态）
   - 显示生成提示词和时间戳
2. **后端**:
   - RenderingService 服务，生成设计 prompt
   - `generate_rendering` 命令，自动解析房间类型和风格
   - 支持：客厅、卧室、厨房、餐厅、卫生间、书房、阳台
   - 支持：现代简约、北欧、中式、日式、欧式、美式、工业、轻奢风格
